...
module New_id(M: Map.S) = struct
  ...
  let make_new_mult m keys =
    let m, old_ids, new_ids = List.fold_left (fun (m, old_ids, new_ids) key ->
...
let translate_pred ctx name arity terms =
  ...
  let pmap, new_id = P.maybe_make_new ctx.pmap (name, arity) in
  let vmap, trsfd = translate_pred_args (List.assoc name ctx.schema) ctx.vmap terms in
  let generic_pred =
...
let rec translate_formula ctx = function
  ...
  | Once (intv, f) ->
    let f, ctx = translate_formula ctx f in
    let intv = translate_intv intv in
    let f = (match if_unbounded intv with
              | Some k -> MOnceU (k, f)
              | None -> MOnceB (intv, f)) in
    f, ctx
  | Eventually (intv, f) ->
    let f, ctx = translate_formula ctx f in
    let intv = translate_intv intv in
...
and transform_fused_op ctx sops = function
  | Exists (vars, f) ->
      let module S = New_id(String_map) in
      let vmap, old_ids, new_ids = S.make_new_mult ctx.vmap vars in
      let f, ctx = transform_fused_op {ctx with vmap} ((MExists new_ids) :: sops) f in
      let vmap = S.update_all vmap vars old_ids in
      f, {ctx with vmap}
  | And (f1, f2) ->
      failwith "rofl"
  | f -> 
...
let make_exformula schema f = 
  let init_ctx = {schema = schema; pmap = Pred_map.empty; vmap = String_map.empty} in
