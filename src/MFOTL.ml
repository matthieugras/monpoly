...
let ts_of_cst c =
  match c with
  | ZInt _
  | Regexp _ -> failwith "[MFOTL.ts_of_cst] conversion not possible"
  | Int i -> i
  | Str s -> int_of_string s
  | Float f -> failwith "[MFOTL.ts_of_cst] conversion not possible"
...
let string_of_ts ts =
  if !unixts then
    let tm = Unix.gmtime (float_of_int ts) in
...
let string_of_interval (a,b) =
  (match a with
    | OBnd a -> Printf.sprintf "(%d," a
    | CBnd a -> Printf.sprintf "[%d," a
    | Inf -> Printf.sprintf "(*,") 
  ^
  (match b with
  | OBnd b -> Printf.sprintf "%d)" b
  | CBnd b -> Printf.sprintf "%d]" b
  | Inf -> Printf.sprintf "*)")
