...
let main () = 
  ...
  let (sigout,fma) = generate_formula ~signature:sigmap ~max_lb:!max_lb ~max_interval:!max_interval ~past_only:!past_only ~all_rels:!all_rels ~aggr:!aggregations ~foo:!fo_only ~ndi:!ndi ~max_const:!max_const ~qtl:!qtl ~nogenlet:!nogenlet !free_vars !size in
...
let _ = 
  Arg.parse [
    ...
    "-nolet", Arg.Set nogenlet, "\t\t\t Do not generate LET formulas (Default: false)";
